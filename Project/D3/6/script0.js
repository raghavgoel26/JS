$( function() {
  ...
  $( function() {
      ...
      var yColumn="Purchasing Power in Billions ( Current International Dollar) - 2013"
      var results,
          data = [],
          chart,
          bars,
      ...
      results.forEach( function( key, val ) {
          ...
          result.ppb = val[yColumn]; //parseInt( val.Population.replace( /,/g, '' ), 10 );
        //  console.log(result);
          data.push( result );
      } );
      ...
      for(i=0;i<l;++i)
      {
        for(j=i+1;j<l;++j)
        {
          if((parseFloat(data[i].ppb))<(parseFloat(data[j].ppb)))
          {
            var temp=data[i];
            data[i]=data[j];
            data[j]=temp;
      ...
      bars.selectAll( 'rect' )
        ...
        .enter().append( 'rect' )
          ...
          .attr( 'y', function( d ) { return (h - margin) - y( d.ppb ) + .5 } )
          .attr( 'width', w )
          .attr( 'height', function( d ) { return y( d.ppb ) } )
          .append('g');
      ...
      yAxis = d3.svg.axis()
          .scale(d3.scale.linear().domain( [0, d3.max( data, function( d ) { return d.ppb; } )] ).rangeRound( [h - margin, 0] ))
          .tickSize(6, 3, 1)
          .orient('left')
          ;

$( function() {
    ...
    var column2="GDP Billions (US$) - 2013"
    ...
    {
    json2[0].pop+=parseFloat(json[i][column1]);
    json2[0].gdp+=parseFloat(json[i][column2]);
    }
    }
    ...
    {
    json2[1].pop+=parseFloat(json[i][column1]);
    json2[1].gdp+=parseFloat(json[i][column2]);
    }
    }
    ...
    {
    json2[2].pop+=parseFloat(json[i][column1]);
    json2[2].gdp+=parseFloat(json[i][column2]);
    }
    }
    ...
    {
    json2[3].pop+=parseFloat(json[i][column1]);
    json2[3].gdp+=parseFloat(json[i][column2]);
    }
    }
    ...
    {
    json2[4].pop+=parseFloat(json[i][column1]);
    json2[4].gdp+=parseFloat(json[i][column2]);
    }
    }
    ...
    {
    json2[5].pop+=parseFloat(json[i][column1]);
    json2[5].gdp+=parseFloat(json[i][column2]);
    }
    }
    ...
    results = d3.map( json2 );

    results.forEach( function( key, val ) {
        var result = {};

        result.conti = val[xColumn];
        result.pop = val[yColumn]; //parseInt( val.Population.replace( /,/g, '' ), 10 );
      //  console.log(result);
        data.push( result );
    } );
console.log("hello");
...
console.log("hello");
    ...
    bars.selectAll( 'rect' )
        .data( data )
      .enter().append( 'rect' )
        .attr( 'x', function( d, i ) { return x( d.conti ) +250; } )
        .attr( 'y', function( d ) { return (h - margin) - y( d.pop ) + .5 } )
        .attr( 'width', w )
    ...
    yAxis = d3.svg.axis()
        .scale(d3.scale.linear().domain( [0, d3.max( data, function( d ) { return d.pop; } )] ).rangeRound( [h - margin, 0] ))
        .tickSize(6, 3, 1)
        .orient('left')
        ;
    ...
    chart.append('g')
        .attr('class', 'x axis')
        .attr('transform', 'translate(154, ' + (h - margin) + ')')
        .call(xAxis);
